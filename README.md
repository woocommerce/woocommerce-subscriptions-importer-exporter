...
Firstly, if you're just importing one tax rate, then you can simply put the tax id (int) or Code (i.e. VAT) in the `tax_items` column without needing to have `code:VAT` or `id:3` - the importer supports both!
...
## Logger
The importer will log all exceptions that were caught during the import and will outline the error which occurred along with the line in the CSV that caused it.

The logs are located at: **WooCommerce > System Status > Logs**  - _wcs-importer-_
![Import Logger](https://cldup.com/uyUAB1Ssuq.png)

This log contains each row from the CSV which failed and the error message or exception that was thrown.
...
This shutdown log file can be found: **WooCommerce > System Status > Logs** - _wcs-importer-shutdown-_

![Importer Shutdown](https://cldup.com/rz7dwYivWN-2000x2000.png)
