...
<div class="total_row tax_row" data-order_item_id="<?php echo $item_id; ?>">
	...
	<p class="first">
		...
		<input type="text" class="order_taxes_amount wc_input_price" name="recurring_order_taxes_amount[<?php echo $item_id; ?>]" placeholder="<?php echo wc_format_localized_price( 0 ); ?>" value="<?php if ( isset( $item['tax_amount'] ) ) echo esc_attr( wc_format_localized_price( $item['tax_amount'] ) ); ?>" />
	</p>
	<p class="last">
		<label><?php _e( 'Shipping Tax:', 'woocommerce-subscriptions' ) ?></label>
		<input type="text" class="order_taxes_shipping_amount wc_input_price" name="recurring_order_taxes_shipping_amount[<?php echo $item_id; ?>]" placeholder="<?php echo wc_format_localized_price( 0 ); ?>" value="<?php if ( isset( $item['shipping_tax_amount'] ) ) echo esc_attr( wc_format_localized_price( $item['shipping_tax_amount'] ) ); ?>" />
	</p>
