...
class WCS_Importer_Simple_Test extends WCS_Importer_UnitTestCase {
	// not used yet but will be used later as I add more test cases
	static $import_results = array();
	// user id of the new user created within import
	static $user_id;
	...
	public function setUp() {
		...
		if( ! isset( self::$user_id ) ) {
			...
			self::$import_results = WCS_Import_Parser::import_data( $test_csv, $mapped_fields, 0, 5000, 1, 'false', 'false' );
			self::$user_id = self::$import_results[0]['user_id'];
		}
	}
	...
	public function test_new_customer_default_username() {
		...
		$actual = ( isset( get_userdata( self::$user_id )->user_login ) ) ? get_userdata( self::$user_id )->user_login : '';
		$this->assertEquals( $expected_username, $actual );
	}
	...
	*/
	public function test_new_customer_meta_from_csv() {
		...
		$this->assertEquals( $expected_email, get_userdata( self::$user_id )->user_email );
	...
	/**
	 * Test the values set in the returned array are correctly set.
	 * Checks values such as edit_user_link, edit_order_link, edit_post_link
	 * @since 1.0.0
	 */
	public function test_other_results_values() {

	}

	/**
	 * Test all the meta data was correctly added to the order for this particular test case
	 * @since 1.0.0
	 */
	public function test_order_meta_correct() {

	}

	/**
	 * Checks the subscription imported is correct.
	 * @since 1.0.0
	 */
	...
	/**
	 * Clean out the static variables so that all the results from the previous test cases are not appended
	 * to the end of WCS_Import_Parser::$results. Cleaning other static variables to be sure.
	 * @since 1.0.0
	 */
	function tearDown() {
		// clean out static variables c
		if( ! empty( WCS_Import_Parser::$results ) ) {
			WCS_Import_Parser::$results = array();
			error_log( '$results array CLEARED' );
		} else if ( ! empty( WCS_Import_Parser::$result ) ) {
			WCS_Import_Parser::$result = array();
			error_log( '$result array CLEARED' );
		}
	}
